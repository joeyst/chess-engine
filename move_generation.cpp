...
namespace GenerateMoves {
  ...
  vector<ARRAY_OF_BOARDS> wstates(ARRAY_OF_BOARDS bitmaps, uint64_t square) {
  ...
  vector<ARRAY_OF_BOARDS> bstates(ARRAY_OF_BOARDS bitmaps, uint64_t square) {
    vector<ARRAY_OF_BOARDS> states = {};
    uint64_t ally_occ = Boards::retrieve_black_occ(bitmaps);
    uint64_t occ = Boards::retrieve_all_occ(bitmaps);
    uint8_t team = BLACK;

    for (auto state : Rook::generate_brook_states(bitmaps, occ, ally_occ, bitmaps[WROOK])) states.push_back(state);
    for (auto state : Queen::generate_bqueen_states(bitmaps, occ, ally_occ, bitmaps[WQUEEN])) states.push_back(state);
    for (auto state : Bishop::generate_bbishop_states(bitmaps, occ, ally_occ, bitmaps[WBISHOP])) states.push_back(state);
    for (auto state : Knight::generate_bknight_states(bitmaps[WKNIGHT], ally_occ, bitmaps)) states.push_back(state);
    for (auto state : Pawn::generate_bpawn_states(bitmaps)) states.push_back(state);
    for (auto state : King::generate_bking_states(bitmaps, bitmaps[WKING], ally_occ)) states.push_back(state);
